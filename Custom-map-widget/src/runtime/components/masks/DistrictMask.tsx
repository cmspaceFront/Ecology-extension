...
export interface DistrictMaskProps {
  map: __esri.Map | null;
...
/**
 * Компонент для создания маски района
 * Затемняет все области кроме выбранного района
 */
const DistrictMask = ({
  map,
  view,
  ...
  useEffect(() => {
    ...
    const initMask = async () => {
      ...
      try {
        // Проверяем кэш района
        const cacheKey = `district_${selectedSoato}`;
        ...
        const soatoFieldName = 'district';
        const isStringField = true;
        ...
        if (soatoFieldName) {
          try {
            ...
            const queryUrl = `${DISTRICT_FEATURE_SERVER_URL}/query?where=${encodeURIComponent(whereClause)}&f=json&outSR=3857&returnGeometry=true&outFields=&maxRecordCount=1`;
        ...
        // Получаем геометрию выбранного района
        ...
        // Рисуем только границу выбранного района (без затемнения)
