...
const BasemapGallery = ({ onChangeBasemap, currentBasemap = 'hybrid', onToggleImageService }: BasemapGalleryProps) => {
  ...
  const getBasemapTitle = (basemapId: string): string => {
    const translationKey = `basemap.basemaps.${basemapId}`;
    const translated = t(translationKey);
    // Если перевод не найден (вернулся ключ), возвращаем оригинальное название
    return translated !== translationKey ? translated : BASEMAPS.vektor.find(b => b.id === basemapId)?.title || BASEMAPS.raster.find(b => b.id === basemapId)?.title || basemapId;
  };
        ...
        <div className="basemap-gallery">
          ...
          <div className="basemap-gallery__content">
            ...
            <div className="basemap-gallery__section">
              <div className="basemap-gallery__section-title">{t('basemap.sections.vector')}</div>
                ...
                return (
                  ...
                  >
                    <div className="basemap-option__content">
                      ...
                      <span className="basemap-option__title">{getBasemapTitle(basemap.id)}</span>
                    </div>
                    {isSelected && (
            ...
            {/* Raster Section */}
            <div className="basemap-gallery__section">
              <div className="basemap-gallery__section-title">{t('basemap.sections.raster')}</div>
            ...
            <div className="basemap-gallery__footer">
              <label className="basemap-toggle-switch">
                <span className="basemap-toggle-switch__label">{t('basemap.imageservice.label')}</span>
                <div className="basemap-toggle-switch__container">
                  <input 
                    type="checkbox" 
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        localStorage.setItem('hybrid-map-widget-imageservice', enabled.toString());
                      } catch {
                    ...
                    className="basemap-toggle-switch__input"
                  />
                  <span className="basemap-toggle-switch__slider"></span>
                </div>
              </label>
            </div>
          </div>
